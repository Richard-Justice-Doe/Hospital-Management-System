    ...
    <div className="p-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="text-xl font-semibold text-clinic-900">{snapshot.title}</h1>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {canOpenBilling && (
            <Link to="/care/billing" className="rounded-lg bg-clinic-700 px-3 py-1.5 text-sm font-medium text-white">
              {user?.role === 'CASHIER' ? 'Receive payment' : 'Collections'}
            </Link>
          )}
          <button
            type="button"
      ...
      <section className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {snapshot.cards.map((card) => (
          <article key={card.key} className="rounded-xl border bg-white p-4">
