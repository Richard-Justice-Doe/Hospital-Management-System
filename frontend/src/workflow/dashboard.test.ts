...
describe('department dashboard', () => {
  it('counts visits, registrations, NHIS, private, and check-ins', () => {
    ...
    expect(snapshot.hospital.visits).toBe(6);
    expect(snapshot.hospital.checkIns).toBe(6);
    expect(snapshot.hospital.registration).toBe(5);
    expect(snapshot.hospital.nhis).toBe(3);
    expect(snapshot.hospital.private).toBe(3);
    ...
    expect(snapshot.trend.reduce((sum, point) => sum + point.visits, 0)).toBe(5);
  });
